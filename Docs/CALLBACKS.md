...
```csharp
Wavedash.SDK.OnLobbyJoined    // Called when player joins a lobby
Wavedash.SDK.OnLobbyLeft      // Called when player leaves a lobby
```
...
1. Unity SDK calls `Wavedash.SDK.Init()`
2. Unity creates a hidden callback receiver GameObject
3. Unity passes its instance to `WavedashJS.setUnityInstance()` on page load
...
void HandleLobbyJoined(Dictionary<string, object> data) {
    string lobbyId = data["id"].ToString();
